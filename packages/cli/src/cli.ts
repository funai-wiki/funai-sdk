...
async function inferStake(_network: CLINetworkAdapter, args: string[]): Promise<string> {
  ...
  const fee = args.length > 4 && args[4] ? BigInt(args[4]) : undefined;
  const nonce = args.length > 5 && args[5] ? BigInt(args[5]) : undefined;
...
async function inferUnlock(_network: CLINetworkAdapter, args: string[]): Promise<string> {
  ...
  const fee = args.length > 1 && args[1] ? BigInt(args[1]) : undefined;
  const nonce = args.length > 2 && args[2] ? BigInt(args[2]) : undefined;
